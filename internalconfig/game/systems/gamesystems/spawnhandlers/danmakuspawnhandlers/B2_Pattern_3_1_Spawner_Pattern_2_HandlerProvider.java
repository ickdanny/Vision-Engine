...
import static internalconfig.game.components.spawns.DanmakuSpawns.B2_PATTERN_3_1_SPAWNER_PATTERN_2;
import static internalconfig.game.systems.gamesystems.spawnhandlers.EnemyProjectileColors.*;
import static internalconfig.game.systems.gamesystems.spawnhandlers.EnemyProjectileTypes.*;

public class B2_Pattern_3_1_Spawner_Pattern_2_HandlerProvider extends AbstractDifficultyDependentSpawnHandlerProvider {

    public B2_Pattern_3_1_Spawner_Pattern_2_HandlerProvider(SpawnBuilder spawnBuilder, AbstractComponentTypeContainer componentTypeContainer) {
    ...
    private class Template extends AbstractPositionSpawnHandler {
        ...
        private Template(int mod, int symmetry, double speed, double angularVelocity, SpawnBuilder spawnBuilder) {
            super(B2_PATTERN_3_1_SPAWNER_PATTERN_2, spawnBuilder, componentTypeContainer);
            this.mod = mod;
        ...
        public void handleSpawn(AbstractECSInterface ecsInterface, int tick, int entityID) {
            if (tickMod(tick, mod)) {
                ...
                SpawnUtil.spiralFormation(
                        B2_PATTERN_3_1_SPAWNER_PATTERN_2.getDuration() - tickExponentialFunction(tick),
                        B2_PATTERN_3_1_SPAWNER_PATTERN_2.getDuration(),
                        baseAngle,
                        angularVelocity,
                        (angle) -> {
        ...
        private int tickExponentialFunction(int tick) {
            return (int) (Math.pow(2, (B2_PATTERN_3_1_SPAWNER_PATTERN_2.getDuration() - tick) / 15d)) - 1;
        }

        private void spawnBullet(DoublePoint pos, AbstractVector velocity, AbstractPublishSubscribeBoard sliceBoard) {
            sliceBoard.publishMessage(
                    spawnBuilder.makeStraightSlowableEnemyBullet(pos, velocity, MEDIUM, ROSE, -200, 1)
