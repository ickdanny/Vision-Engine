...
public class S5_Wave_1_Flame_Spawner_SpawnHandler extends AbstractSpawnHandlerTemplate {
    ...
    private static final InstructionNode<?, ?>[] PROGRAM = ProgramBuilder.linearLink(
            ...
            new InstructionNode<>(SPEED_UP_TO_VELOCITY, new Tuple2<>(new PolarVector(FINAL_SPEED, 135), SPEED_DURATION))
    ).compile();


    public S5_Wave_1_Flame_Spawner_SpawnHandler(SpawnBuilder spawnBuilder) {
        super(StageSpawns.S5_WAVE_1_FLAME_SPAWNER, spawnBuilder);
    }
    ...
    public void handleSpawn(AbstractECSInterface ecsInterface, int tick, int entityID) {
        if (tickMod(tick, MOD)) {
            ...
            DoublePoint pos = new DoublePoint(RIGHT_OUT - 20, y);
            AbstractVector initVelocity = new PolarVector(speed, -135);
