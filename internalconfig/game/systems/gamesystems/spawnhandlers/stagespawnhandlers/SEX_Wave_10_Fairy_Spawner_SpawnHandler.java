...
import static internalconfig.game.components.spawns.DanmakuSpawns.SEX_WAVE_10_FAIRY_RING;
import static internalconfig.game.components.spawns.StageSpawns.SEX_WAVE_10_FAIRY_SPAWNER;
import static internalconfig.game.systems.gamesystems.spawnhandlers.SpawnUtil.*;

public class SEX_Wave_10_Fairy_Spawner_SpawnHandler extends AbstractSpawnHandlerTemplate {

    private static final int MOD = SEX_WAVE_10_FAIRY_SPAWNER.getDuration() / 8;

    private static final int HEALTH = 2000;
    private static final double INIT_SPEED = 4.5;
    ...
    private static final int WAIT_AFTER_DURATION = 80;
    ...
    private static final InstructionNode<?, ?>[] PROGRAM = ProgramUtil.makeShootOnceAndLeaveEnemyProgram(
            ...
            SLOW_DURATION,
            WAIT_DURATION,
            SEX_WAVE_10_FAIRY_RING,
            WAIT_AFTER_DURATION,
            new PolarVector(FINAL_SPEED, new Angle(90)),
            SPEED_DURATION
    ).compile();

    public SEX_Wave_10_Fairy_Spawner_SpawnHandler(SpawnBuilder spawnBuilder) {
        super(SEX_WAVE_10_FAIRY_SPAWNER, spawnBuilder);
    }
    ...
    public void handleSpawn(AbstractECSInterface ecsInterface, int tick, int entityID) {
        if (tickMod(tick, MOD)) {
            ...
            SpawnUtil.staggeredBlockFormation(
                    tick,
                    SEX_WAVE_10_FAIRY_SPAWNER.getDuration(),
                    MOD,
