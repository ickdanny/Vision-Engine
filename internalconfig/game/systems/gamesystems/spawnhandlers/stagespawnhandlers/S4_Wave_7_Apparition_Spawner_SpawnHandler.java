...
import static internalconfig.game.components.spawns.StageSpawns.S4_WAVE_7_APPARITION_SPAWNER;
import static internalconfig.game.components.Instructions.*;

public class S4_Wave_7_Apparition_Spawner_SpawnHandler extends AbstractSpawnHandlerTemplate {

    private static final int MOD = S4_WAVE_7_APPARITION_SPAWNER.getDuration()/4;
    ...
    private static final int Y_LOW = -21;
    ...
    public S4_Wave_7_Apparition_Spawner_SpawnHandler(SpawnBuilder spawnBuilder) {
        super(S4_WAVE_7_APPARITION_SPAWNER, spawnBuilder);
    }
    ...
    public void handleSpawn(AbstractECSInterface ecsInterface, int tick, int entityID) {
        if (tickMod(tick, MOD)) {
            ...
            double x = getX(random, tick * (tick + 12));
            double y = Y_LOW + (tick * Y_TICK_MULTI);
            DoublePoint pos = new DoublePoint(x, y);

            if (tickMod(tick, MOD * 2)) {
                spawnApparition(sliceBoard, pos, makeProgram(S4_WAVE_7_APPARITION_RINGS));
            } else {
                spawnApparition(sliceBoard, pos, makeProgram(S4_WAVE_7_APPARITION_ARCS));
            }
        }
    }
