...
import static internalconfig.game.components.spawns.StageSpawns.S4_WAVE_8_APPARITION_SPAWNER;

public class S4_Wave_8_Apparition_Spawner_SpawnHandler extends AbstractSpawnHandlerTemplate {

    private static final int MOD = S4_WAVE_8_APPARITION_SPAWNER.getDuration()/5;
    private static final int HEALTH = 9000;
    ...
    public S4_Wave_8_Apparition_Spawner_SpawnHandler(SpawnBuilder spawnBuilder) {
        super(S4_WAVE_8_APPARITION_SPAWNER, spawnBuilder);
    }
    ...
    public void handleSpawn(AbstractECSInterface ecsInterface, int tick, int entityID) {
        if (tickMod(tick, MOD)) {
            ...
            double y = Y_LOW + ((S4_WAVE_8_APPARITION_SPAWNER.getDuration() - tick) * Y_TICK_MULTI);
            DoublePoint pos = new DoublePoint(x, y);

            if (!tickMod(tick, MOD * 2)) {
                spawnApparition(sliceBoard, pos, makeProgram(S4_WAVE_8_APPARITION_RINGS));
            } else {
                spawnApparition(sliceBoard, pos, makeProgram(S4_WAVE_8_APPARITION_COLUMNS));
            }
        }
    }
