...
public class SEX_Wave_8_Smokeball_Spawner_SpawnHandler extends AbstractSpawnHandlerTemplate {

    private static final int MOD = 40;
    private static final int HEALTH = 1000;
    private static final double SPEED = 3.6;

    private static final double Y1 = 65;
    private static final double Y2 = 125;

    public SEX_Wave_8_Smokeball_Spawner_SpawnHandler(SpawnBuilder spawnBuilder) {
        super(StageSpawns.SEX_WAVE_8_SMOKEBALL_SPAWNER, spawnBuilder);
    }
    ...
    public void handleSpawn(AbstractECSInterface ecsInterface, int tick, int entityID) {
        if (tickMod(tick, MOD)) {
            ...
            if(tickMod(tick, MOD * 2)){
                pos = new DoublePoint(LEFT_OUT, Y2);
                angle = new Angle(0);
            }
            else {
                pos = new DoublePoint(RIGHT_OUT, Y1);
                angle = new Angle(180);
            }
    ...
    private InstructionNode<?, ?>[] makeProgram(Random random) {
        ...
        return ProgramBuilder.linearLink(
                ...
                new InstructionNode<>(SET_SPAWN, SEX_WAVE_8_SMOKEBALL_SPIRAL)
        ).compile();
    }
}