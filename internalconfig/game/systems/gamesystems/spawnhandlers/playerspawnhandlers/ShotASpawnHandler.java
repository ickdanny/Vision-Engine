...
class ShotASpawnHandler extends AbstractShotSpawnHandler {

    public ShotASpawnHandler(SpawnBuilder spawnBuilder, AbstractComponentTypeContainer componentTypeContainer) {
    ...
    void spawnUnfocused_MAX(AbstractECSInterface ecsInterface, int tick, int entityID) {
        ...
        if(spawnBasic || spawnSpecial){
            ...
            if(spawnSpecial){
                spawnSpecialPairNormalFar(pos, sliceBoard);
            }
        }
    }
    ...
    void spawnFocused_8(AbstractECSInterface ecsInterface, int tick, int entityID) {
        if(tickMod(tick, 6)) {
            ...
            //mod 30
            if(tickMod(tick, 5)){
                spawnSpecialPairHighClose(pos, sliceBoard);
            }
        }
    }
    ...
    void spawnFocused_MAX(AbstractECSInterface ecsInterface, int tick, int entityID) {
        ...
        if(spawnBasic || spawnSpecial){
            ...
            if(spawnBasic) {
                ...
                SpawnUtil.mirrorFormation(
                        BASIC_SHOT_SPAWN_OFFSET.multiAdd(4, pos),
                        pos.getX(),
                        (p) -> spawnBasicBullet(p, UP_ANGLE, sliceBoard)
                );
            }
            if(spawnSpecial){
                spawnSpecialPairHighFar(pos, sliceBoard);
            }
        }
    }
}