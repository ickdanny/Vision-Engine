...
class SpeedUpAndTurnToVelocityInstructionHandler extends AbstractSpeedUpAndTurnToVelocityInstructionHandler {

    SpeedUpAndTurnToVelocityInstructionHandler(AbstractComponentTypeContainer componentTypeContainer) {
        super(componentTypeContainer);
    }

    @Override
    public Instructions<Tuple3<AbstractVector, Angle, Integer>, Tuple2<Double, Double>> getInstruction() {
        return Instructions.SPEED_UP_AND_TURN_TO_VELOCITY;
    }
    ...
    protected Tuple2<Double, Double> retrieveOrCreateVelocityIncrement(
        ...
        if (dataMap.containsKey(node)) {
            ...
            double angleDifference = finalAngle.smallerDifference(initAngle);
